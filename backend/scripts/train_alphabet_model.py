...
from app.services.alphabet_dataset import get_alphabet_dataset_status  # noqa: E402
...
def extract_features_from_image(image_bgr: np.ndarray, hands: mp.solutions.hands.Hands) -> np.ndarray | None:
...
def collect_dataset(
    collated_dir: Path, max_images_per_class: int | None, use_mirror_augmentation: bool
) -> tuple[np.ndarray, np.ndarray, dict[str, int], int]:
    class_dirs = sorted([item for item in collated_dir.iterdir() if item.is_dir()], key=lambda item: item.name)
    labels: list[str] = []
    ...
    with mp.solutions.hands.Hands(
        ...
        for class_dir in class_dirs:
            ...
            kept = 0
            for image_path in tqdm(image_paths, desc=f"Extracting {class_dir.name}", leave=False):
            ...
            class_kept[class_dir.name] = kept

    if not features:
        raise RuntimeError("No features extracted from dataset. Training cannot continue.")

    return np.stack(features), np.array(labels), class_kept, skipped_no_hand


def main() -> None:
    parser = argparse.ArgumentParser(description="Train alphabet model from Kaggle Collated dataset.")
    ...
    parser.add_argument("--model-out", type=str, default="artifacts/alphabet_model.joblib")
    parser.add_argument("--report-out", type=str, default="artifacts/alphabet_training_report.json")
    args = parser.parse_args()

    status = get_alphabet_dataset_status()
    if not status["kaggle_collated_found"]:
        raise RuntimeError(f"Collated dataset folder not found at {status['kaggle_collated_path']}")

    collated_dir = Path(status["kaggle_collated_path"])
    ...
    print(f"Using collated dataset: {collated_dir}")
    ...
    report_payload = {
        ...
        "dataset_collated_path": str(collated_dir),
